...
class Main
  ...
  def choose_route_stations
    ...
    raise "Введите номера разных станций" if first_id == last_id
    route_stations = []
  ...
  def create_wagon
    loop do
      ...
      puts '2 - Пасажирский'
      puts '3 - Выйти в меню'
      create_wagon_go(firm)
    end
  end

  def create_wagon_go(firm)
    case gets.chomp.to_i
    when 1 then create_cargo_wagon(firm)
    when 2 then create_passenger_wagon(firm)
    when 3 then menu
    end
  end

  def create_cargo_wagon(firm)
    # puts 'Введите название производителя'
    # firm = gets.chomp.to_s
    ...
    @wagons << wagon
    puts "Создан вагон #{wagon} с объемом #{space}"
  rescue RuntimeError => e
    puts e.inspect
    create_wagon
  end

  def create_passenger_wagon(firm)
    puts 'Укажите количество мест в вагоне'
    seats = gets.chomp.to_i
    wagon = PassengerWagon.new(firm, seats)
    @wagons << wagon
    puts "Создан вагон #{wagon} с количеством мест #{seats}"
  rescue RuntimeError => e
    puts e.inspect
    create_wagon
  end
  ...
  def validate_values
    ...
    when 3 then routes_validate
    when 4 then wagons_validate 
    end 
  rescue RuntimeError => e
    puts e.message 
    menu 
