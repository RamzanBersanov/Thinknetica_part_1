class Train
  ...
  def wagons_list 
    ...
    self.wagons.each {|wagon|}
  end 
  
  def wagons_list_info
    #для каждого поезда на станции выводить список вагонов в формате: Номер вагона (можно назначать автоматически), тип вагона, кол-во свободных и занятых мест (для пассажирского вагона) или кол-во свободного и занятого объема (для грузовых вагонов)
    self.wagons.each_with_index do |wagon, index|
      if wagon.wagon_type == :cargo
        puts "Номер вагона: #{index}"
        puts "Тип - грузовой"
        puts "Доступный объем - #{wagon.free_space}"
        puts "Занятый объем - #{wagon.taken_space}" 
      elsif wagon.wagon_type == :passenger
        puts "Номер вагона: #{index}"
        puts "Тип - пассажирский"
        puts "Свободные места - #{wagon.free_seats}"
        puts "Занятые места - #{wagon.taken_seats}" 
      else
      end 
    end 
  end
